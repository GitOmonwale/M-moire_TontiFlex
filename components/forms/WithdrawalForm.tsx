...
const WithdrawalForm: React.FC<WithdrawalFormProps> = ({
  ...
  const validateForm = () => {
    ...
    if (parseFloat(retraitData.montant) > parseFloat(participantDetails.solde_disponible)) {
    ...
    if (retraitData.numero_telephone && !/^(\+229|229)?[0-9]{8}$/.test(retraitData.numero_telephone.replace(/\s/g, ''))) {
  ...
  const handlePhoneChange = (value: string) => {
    setRetraitData(prev => ({ ...prev, numero_telephone: value }));
  ...
  const soldeDisponible = parseFloat(participantDetails.solde_disponible);
