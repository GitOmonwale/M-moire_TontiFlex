...
const SavingsAccountsOverview = () => {
  const { fetchMyAccount, loading, error } = useSavingsAccounts();
  const [accounts, setAccounts] = useState<MyAccountSummary[]>([]);
  ...
  useEffect(() => {
    const loadAccounts = async () => {
      try {
        const accountData = await fetchMyAccount();
        if (accountData) {
  ...
  const stats = {
    totalSolde: accounts.reduce((sum, acc) => sum + Number(acc.solde || 0), 0),
    totalDepose: accounts.reduce((sum, acc) => sum + Number(acc.totalDepose || 0), 0),
    totalRetire: accounts.reduce((sum, acc) => sum + Number(acc.totalRetire || 0), 0),
          ...
          <GlassCard className="p-4 mb-6 bg-white/50 border-l-4 border-blue-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-medium text-gray-900">Conseil :                 Diversifiez vos comptes en ouvrant des comptes dans différents SFD pour réduire les risques.</p>
              </div>
            </div>
          </GlassCard>
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">Impossible de charger les comptes. Veuillez réessayer.</p>
              <button
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                      <p className="text-sm text-gray-500">
                        Dernier mouvement: {account.derniereMouvement ? 
                          format(new Date(account.derniereMouvement), 'dd MMM à HH:mm', { locale: fr }) : 
                          'Aucun mouvement'}
                      </p>
                      <div className="flex gap-2">
                        ...
                        <Link href={`/dashboards/dashboard-client/saving-accounts/${account.id}`}>
                          <GlassButton size="sm">
